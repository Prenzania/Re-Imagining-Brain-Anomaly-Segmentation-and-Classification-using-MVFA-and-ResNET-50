...
def main():
    ...
    for epoch in range(args.epoch):
        ...
        epoch_losses.append(mean_loss)  # Store the mean loss for this epoch
        ...
        result = test(args, model, text_features, seg_mem_features, det_mem_features, test_loader)

        if result > best_result:
            best_result = result

        print(f'Best AUC: {best_result}')

        if args.save_model:
            save_path = os.path.join(args.save_path, args.obj + '_epoch_' + str(epoch) + '.pth')
    ...
    plt.plot(range(1, args.epoch + 1), epoch_losses, marker='o')
