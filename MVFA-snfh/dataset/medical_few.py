...
class MedDataset(Dataset):
    ...
    #     print(f"Total normal images: {len(normal_names)}")

    #     # select images
    #     if self.iterate < 0:
    #         if len(normal_names) < self.shot: 
    #             raise ValueError("Not enought normal images")
    ...
    def get_few_normal(self):
        ...
        #print(f"Total normal images found: {len(normal_names)}") 
        
        #filter only valid img files 
        valid_img = [f for f in normal_names if f.endswith('.png')]
        #print(f"Filtered valid imgs: {valid_img}")

        # select images
        #changed normal_names to valid_img
        if self.iterate < 0:
            if len(valid_img) < self.shot:
                raise ValueError("Not enough normal images available.")
            random_choice = random.sample(valid_img, self.shot)
        else:
            random_choice = []
            try:
                with open(f'./dataset/fewshot_seed/{self.class_name}/{self.shot}-shot.txt', 'r', encoding='utf-8') as infile:
        ...
        #print(f"Randomly selected normal images: {random_choice}")  # Debugging info
        ...
        if len(x) < self.shot:
            raise ValueError("Not enough images found after filtering.")
    ...
    def get_few_abnormal(self):
        ...
        #print(f"Total abnormal images found: {len(abnormal_names)}")  # Debugging info

        # select images
        if self.iterate < 0:
            if len(abnormal_names) < self.shot:
                raise ValueError("Not enough abnormal images available.")
            random_choice = random.sample(abnormal_names, self.shot)
        else:
            random_choice = []
            try:
        ...
        #print(f"Randomly selected abnormal images: {random_choice}")  # Debugging info
