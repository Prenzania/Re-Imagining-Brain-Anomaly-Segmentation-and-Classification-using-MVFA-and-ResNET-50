...
def test(args, model, test_loader, text_features, seg_mem_features, det_mem_features):
    ...
    if CLASS_INDEX[args.obj] > 0:
        ...
        for index, image in enumerate(seg): 
            ...
            # # if(test_images[index].split('/')[6] == 'BraTS-MEN-00129-000_ntc.png'):
            # #     print(image_norm)
            
            # image_norm = np.where(image_norm >= 50, 255, 0).astype(np.uint8)
            fil = test_images[index].split('/')[6]
            
            image_norm = (image * 255).astype(np.uint8)
            heat_map_img = Image.fromarray(image_norm)
